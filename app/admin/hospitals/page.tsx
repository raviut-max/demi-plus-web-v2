...
export default function HospitalsPage() {
  ...
  // ✅ ฟังก์ชันจัดกลุ่มโรงพยาบาล (กรองตามสิทธิ์การเข้าถึง)
  const groupHospitals = (allHospitals: any[], accessibleIds: string[]) => {
    console.log('📊 [groupHospitals] Grouping', allHospitals.length, 'hospitals...');
    
    // ✅ กรองเฉพาะแม่ข่ายที่ผู้ใช้มีสิทธิ์เห็น
    let mainHospitals = allHospitals.filter(h => h.type === 'main');
    
    // ✅ ถ้าไม่ใช่ Super Admin → แสดงเฉพาะแม่ข่ายที่ตัวเองดูแล
    if (!isSuperAdmin(user) && accessibleIds.length > 0) {
      mainHospitals = mainHospitals.filter(h => accessibleIds.includes(h.id));
      console.log('🔒 [groupHospitals] Hospital Admin - showing only own main hospitals:', mainHospitals.length);
    }
  ...
  // ✅ ตรวจสอบว่าผู้ใช้มีสิทธิ์เพิ่มลูกข่ายในแม่ข่ายนี้หรือไม่
  const canAddSubHospital = (mainHospitalId: string): boolean => {
    // ✅ Super Admin เพิ่มได้ทุกแม่ข่าย
    if (isSuperAdmin(user)) {
      return true;
    }
    // ✅ Hospital Admin เพิ่มได้เฉพาะแม่ข่ายของตัวเอง
    if (userHospital && userHospital.id === mainHospitalId) {
      return true;
    }
    return false;
  };
  ...
  // ✅ คำนวณจำนวนโรงพยาบาล (เฉพาะที่เข้าถึงได้)
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="space-y-8">
          {groupedHospitals.map((group) => {
            // ✅ ตรวจสอบว่าผู้ใช้มีสิทธิ์เพิ่มลูกข่ายในแม่ข่ายนี้หรือไม่
            const canAddSub = canAddSubHospital(group.id);
                ...
                <div className="p-6">
                  {group.type === 'orphan' ? (
                    ...
                    <div>
                      ...
                      {/* ✅ ปุ่มเพิ่มลูกข่าย - แสดงเฉพาะในแม่ข่ายที่ผู้ใช้มีสิทธิ์เพิ่มได้ */}
                      {canAddSub && (
