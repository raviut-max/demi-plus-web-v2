...
import { ArrowLeft, LogOut, Save, AlertCircle, CheckCircle, MapPin, Hospital, Building2, UserCog } from 'lucide-react';
...
export default function EditPatientPage() {
  ...
  const loadHospitalsWithNetwork = async (userData: any) => {
    try {
      ...
      // ✅ 3. โหลดโค้ชจากเครือข่ายโรงพยาบาลเดียวกัน
                      ...
                      <div className="space-y-1 text-xs">
                        <div className="flex items-center gap-1 text-gray-600">
                          ...
                          <span className="truncate">
                            {hospitals.find(h => h.id === user.hospital_id)?.name || 'ไม่ได้กำหนด'}
                          </span>
                        </div>
                        {user.hospital_id && hospitals.find(h => h.id === user.hospital_id)?.type === 'sub' && (
                          <div className="flex items-center gap-1 text-green-600">
                            <Building2 className="w-3 h-3" />
                            <span>
                              ลูกข่าย: {hospitals.find(h => h.id === userHospitalNetwork.mainHospitalId)?.name}
                            </span>
                          </div>
                        )}
                      </div>
