...
// ✅ การแก้ไข:
//    1. ลบการตรวจสอบสิทธิ์ (Access Control) ออก ให้ทุกบทบาทเข้าใช้งานได้
//    2. ปรับ Dropdown โรงพยาบาล: แสดงเฉพาะเครือข่าย (แม่ข่าย+ลูกข่าย) ของผู้ใช้
//    3. เพิ่ม Dropdown โค้ช: แสดงเฉพาะโค้ชในโรงพยาบาลเครือข่าย พร้อมชื่อรพ. และเรียงลำดับ A-Z
//    4. รักษาโครงสร้าง ฟอร์ม และ validation เดิมไว้ทั้งหมด
...
import { ArrowLeft, LogOut, Save, AlertCircle, CheckCircle, MapPin, Building2 } from 'lucide-react';
...
export default function EditPatientPage() {
  ...
  // State สำหรับโรงพยาบาลและโค้ช (Network-based)
  ...
  useEffect(() => {
    ...
    // ✅ 1. ลบการตรวจสอบสิทธิ์ออก (อนุญาตทุกบทบาท)
    setUser(userData);
    loadPatientData();
    loadNetworkData(userData);
  }, [router]);

  // ✅ 2. โหลดข้อมูลโรงพยาบาลและโค้ชตามเครือข่ายของผู้ใช้
  const loadNetworkData = async (userData: any) => {
    try {
      const allHospitals = await getHospitalsWithHierarchy();
      
      // หาโรงพยาบาลของผู้ใช้ (จาก session หรือ metadata)
      const userHospId = userData.hospital_id;
      const userHosp = allHospitals.find(h => h.id === userHospId);
      
      let networkHospitals = allHospitals;
      if (userHosp) {
        // หา root/main hospital ของเครือข่าย
        const rootId = userHosp.type === 'main' ? userHosp.id : userHosp.parent_id;
        if (rootId) {
          // กรองเฉพาะแม่ข่ายนี้ และลูกข่ายทั้งหมดที่อยู่ภายใต้แม่ข่ายนี้
          networkHospitals = allHospitals.filter(h => h.id === rootId || h.parent_id === rootId);
        }
      }
      setFilteredHospitals(networkHospitals);

      // ✅ 3. โหลดโค้ชเฉพาะในโรงพยาบาลเครือข่าย
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                <input type="text" value={formData.id_card} onChange={handleChange} maxLength={13} className={`w-full px-4 py-2 border rounded-lg ${validationErrors.id_card ? 'border-red-500 bg-red-50' : validationSuccess.id_card ? 'border-green-500 bg-green-50' : 'border-gray-300'}`} placeholder="กรอกเลขบัตรประชาชน 13 หลัก" />
              ...
              <div>
                ...
                <input type="text" required value={formData.first_name} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">นามสกุล *</label>
                <input type="text" required value={formData.last_name} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">HN (Hospital Number) *</label>
                <input type="text" required value={formData.hospital_number} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">วันเกิด *</label>
                <div className="grid grid-cols-3 gap-2">
                  <select value={formData.birth_day} onChange={handleChange} required className="px-2 py-2 border border-gray-300 rounded-lg text-sm">
                    <option value="">วัน</option>
                    {Array.from({ length: 31 }, (_, i) => i + 1).map((day) => (<option key={day} value={day}>{day}</option>))}
                  </select>
                  <select value={formData.birth_month} onChange={handleChange} required className="px-2 py-2 border border-gray-300 rounded-lg text-sm">
                    <option value="">เดือน</option>
                    {THAI_MONTHS.map((month, index) => (<option key={index + 1} value={index + 1}>{month}</option>))}
                  </select>
                  <select value={formData.birth_year} onChange={handleChange} required className="px-2 py-2 border border-gray-300 rounded-lg text-sm">
              ...
              <div>
                ...
                <select value={formData.gender} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg">
              ...
              <div>
                ...
                <input type="tel" value={formData.phone} onChange={handleChange} placeholder="เช่น 0812345678" className={`w-full px-4 py-2 border rounded-lg ${validationErrors.phone ? 'border-red-500' : validationSuccess.phone ? 'border-green-500' : 'border-gray-300'}`} />
              ...
              <div className="col-span-2">
                ...
                <input type="email" value={formData.email} onChange={handleChange} placeholder="patient@example.com" className={`w-full px-4 py-2 border rounded-lg ${validationErrors.email ? 'border-red-500' : validationSuccess.email ? 'border-green-500' : 'border-gray-300'}`} />
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <input type="number" step="0.1" value={formData.current_weight} onChange={handleChange} placeholder="เช่น 65" className={`w-full px-4 py-2 border rounded-lg ${validationErrors.current_weight ? 'border-red-500' : validationSuccess.current_weight ? 'border-green-500' : 'border-gray-300'}`} />
              ...
              <div>
                ...
                <input type="number" step="0.1" value={formData.height} onChange={handleChange} placeholder="เช่น 170" className={`w-full px-4 py-2 border rounded-lg ${validationErrors.height ? 'border-red-500' : validationSuccess.height ? 'border-green-500' : 'border-gray-300'}`} />
              ...
              <div>
                ...
                <input type="number" step="0.1" value={formData.waist_circumference} onChange={handleChange} placeholder="เช่น 85" className={`w-full px-4 py-2 border rounded-lg ${validationErrors.waist_circumference ? 'border-red-500' : validationSuccess.waist_circumference ? 'border-green-500' : 'border-gray-300'}`} />
              ...
              <div>
                ...
                <select value={formData.diabetes_type} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg">
              ...
              <div>
                ...
                <input type="number" step="0.1" value={formData.blood_sugar} onChange={handleChange} placeholder="เช่น 110" className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">ค่า HbA1c</label>
                <input type="number" step="0.1" value={formData.hba1c_level} onChange={handleChange} placeholder="เช่น 7.5" className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
              </div>
              <div className="col-span-3">
                <label className="block text-sm font-medium text-gray-700 mb-1">หมายเหตุ (คำแนะนำเพิ่มเติม)</label>
                <input type="text" value={formData.notes} onChange={handleChange} placeholder="เช่น แพ้ถั่ว แพ้นม (เว้นว่างได้ถ้าไม่มี)" className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <input type="text" value={formData.house_number} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg" placeholder="123" />
                </div>
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-1">ที่อยู่เพิ่มเติม (ถ้ามี)</label>
                  <input type="text" value={formData.address_line1} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg" placeholder="เช่น อพาร์ทเมนท์, อาคาร, ชั้น" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">หมู่ที่/ชุมชน</label>
                  <input type="text" value={formData.village_no} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg" placeholder="หมู่ 5" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">หมู่บ้าน</label>
                  <input type="text" value={formData.village_name} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg" placeholder="หมู่บ้านสุขใจ" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">ซอย</label>
                  <input type="text" value={formData.soi} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg" placeholder="ซอย 5" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">ถนน</label>
                  <input type="text" value={formData.road} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg" placeholder="ถนนสุขุมวิท" />
                </div>
              </div>
              ...
              {/* ✅ Dropdown โรงพยาบาล: แสดงเฉพาะเครือข่ายผู้ใช้ */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">🏥 โรงพยาบาลสังกัด</label>
                <select value={formData.hospital_id} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 max-h-64 overflow-y-auto">
                  <option value="">-- เลือกโรงพยาบาล --</option>
                  {filteredHospitals.map((hospital) => (
                    <optgroup key={hospital.id} label={`🏥 ${hospital.name} (${hospital.code})`}>
                      <option value={hospital.id}>└ {hospital.name} ({hospital.code}) - {hospital.type === 'main' ? 'แม่ข่าย' : 'ลูกข่าย'}</option>
                    </optgroup>
                  ))}
                </select>
              </div>

              {/* ✅ Dropdown โค้ช: แสดงเฉพาะโค้ชในเครือข่าย พร้อมชื่อรพ. และเรียงลำดับ */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">👨‍⚕️ โค้ชผู้ดูแล</label>
                <select name="coach_id" value={formData.coach_id} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 max-h-64 overflow-y-auto">
                  <option value="">-- เลือกโค้ช --</option>
                  {coaches.map((coach) => (
                    <option key={coach.id} value={coach.id}>
                      {coach.name} | {coach.hospitals?.name || 'ไม่ระบุโรงพยาบาล'}
                    </option>
                  ))}
                </select>
                <p className="text-xs text-gray-500 mt-1">แสดงเฉพาะโค้ชในโรงพยาบาลเครือข่ายของคุณ (เรียงตามชื่อ)</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="col-span-2">
                ...
                <input type="text" value={formData.emergency_contact_name} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">เบอร์โทรศัพท์</label>
                <input type="tel" value={formData.emergency_contact_phone} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
              </div>
              <div className="col-span-3">
                <label className="block text-sm font-medium text-gray-700 mb-1">ความสัมพันธ์</label>
                <input type="text" value={formData.emergency_contact_relationship} onChange={handleChange} placeholder="เช่น พ่อ, แม่, สามี" className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-4">
            ...
            <button type="button" onClick={() => router.push(`/admin/patients/${patientId}`)} className="flex-1 bg-gray-500 text-white font-bold py-4 rounded-xl hover:bg-gray-600 transition-all">
              ยกเลิก
            </button>
          </div>
        </form>
