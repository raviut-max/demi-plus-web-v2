...
import {
  ...
  Hospital, Phone, Mail, Stethoscope, AlertCircle, CheckCircle
} from 'lucide-react';
...
export default function AddStaffPage() {
  ...
  const loadHospitals = async () => {
    try { setHospitals(await getHospitalsWithHierarchy()); } catch (e) { console.error(e); }
  };

  const loadAccessibleHospitals = async (userId: string) => {
    try { setAccessibleHospitalIds(await getAccessibleHospitalIds(userId)); } catch (e) { console.error(e); }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ ส่ง Plain Text ไปยัง addStaff (ไม่มีการ Hash)
      const result = await addStaff({
        id_card: formData.id_card,
        password: password,
        full_name_th: formData.full_name_th.trim(),
        role: formData.role,
        specialization_th: formData.specialization_th || undefined,
        ...
        is_temporary_id: false, // ✅ เจ้าหน้าที่ปกติ ไม่ใช่ชั่วคราว
      });
  ...
  const availableHospitals = isSuper ? hospitals : hospitals.filter((h: any) => accessibleHospitalIds.includes(h.id));
          ...
          <div className="bg-blue-50 border-2 border-blue-400 rounded-xl p-4 mb-6 text-left">
            <div className="flex items-start gap-2 mb-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-700 mt-2">💡 รหัสผ่าน = วัน-เดือน-ปีเกิด (dd-mm-yyyy)</p>
              </div>
            </div>
          ...
          <div className="space-y-3">
            <button
              onClick={() => {
                ...
                setFormData({ id_card: '', birth_day: '', birth_month: '', birth_year: '', full_name_th: '', role: 'doctor', specialization_th: '', phone: '', email: '', hospital_id: '', admin_type: null });
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  <Shield className="w-4 h-4 inline mr-1" /> ID Card / เลขบัตรประชาชน *
                </label>
                <input
                  type="text"
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <select value={formData.birth_year} onChange={(e) => setFormData({ ...formData, birth_year: e.target.value })} required className="px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500">
                  ...
                  {Array.from({ length: 80 }, (_, i) => 2567 - i).map((y) => <option key={y} value={y}>{y}</option>)}
                </select>
              </div>
            </div>
            ...
            <div>
              ...
              <input type="text" value={formData.full_name_th} onChange={(e) => setFormData({ ...formData, full_name_th: e.target.value })} required className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500" placeholder="เช่น สมชาย ใจดี" />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input type="text" value={formData.specialization_th} onChange={(e) => setFormData({ ...formData, specialization_th: e.target.value })} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500" placeholder={formData.role === 'osm' ? 'เช่น อาสาสมัครสาธารณสุขประจำหมู่บ้าน' : formData.role === 'helper' ? 'เช่น เจ้าหน้าที่สาธารณสุข, พยาบาล' : 'เช่น อายุรกรรม, ศัลยกรรม'} />
              </div>
            </div>
            ...
            {showAdminTypeField && isSuper && (
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                ...
                <select value={formData.admin_type || ''} onChange={(e) => setFormData({ ...formData, admin_type: (e.target.value as 'super' | 'hospital') || null })} required className="w-full px-4 py-3 border border-purple-300 rounded-xl focus:ring-2 focus:ring-purple-500">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                <Hospital className="w-4 h-4 inline mr-1" /> โรงพยาบาลสังกัด {formData.role !== 'admin' ? '*' : ''}
              </label>
              <select value={formData.hospital_id} onChange={(e) => setFormData({ ...formData, hospital_id: e.target.value })} required={formData.role !== 'admin' || !isSuper} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 max-h-64 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2"><Phone className="w-4 h-4 inline mr-1" /> เบอร์โทรศัพท์</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2"><Mail className="w-4 h-4 inline mr-1" /> อีเมล</label>
            ...
            <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-blue-500 to-cyan-500 text-white font-bold py-4 rounded-xl hover:from-blue-600 hover:to-cyan-600 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
              {loading ? <><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div> กำลังบันทึก...</> : <><Save className="w-5 h-5" /> บันทึกบุคลากร</>}
            </button>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm text-yellow-700">
                  ...
                  <ul className="space-y-1">
                    <li>• รหัสผ่านจะถูกเก็บแบบ Plain Text ตามการตั้งค่าระบบ</li>
                    <li>• รหัสผ่าน = วัน-เดือน-ปีเกิด (dd-mm-yyyy)</li>
